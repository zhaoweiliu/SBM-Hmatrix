         ...
         void *            arg )
{
    ...
    for ( colj = 0; colj < m; colj++ )
    {
        ...
        for ( rowi = 0; rowi < n; rowi++ )
        {
            ...
            matrix[(colj*n) + rowi].re = -value;
            matrix[(colj*n) + rowi].im = 0;
        }
    }
}
...
main ( int argc, char ** argv )
{
    ...
    hlib_clt_print_ps( ct, "cbem1d_ct", & info );                        CHECK_INFO;
    ...
    hlib_bct_print_ps( bct, "cbem1d_bct", & info );                      CHECK_INFO;

    printf( "building BEM matrix using ACA+ ...\n" );
    start = hlib_walltime();

    A = hlib_matrix_build_ccoeff( bct, kernel, & h, HLIB_LRAPX_ACAPLUS,
                                  acc, 1, & info );                         CHECK_INFO;
    
    printf( "  done in %.1f seconds\n", hlib_walltime() - start );
    hlib_matrix_print_ps( A, "cbem1d_A", HLIB_MATIO_SVD, & info );       CHECK_INFO;
    ...
    if ( n < 20 )
    {
        hlib_complex_t *  D   = (hlib_complex_t *) malloc( n * n * sizeof(hlib_complex_t) );
        ...
        A_D = hlib_matrix_import_cdense( n, n, D, 0, & info );             CHECK_INFO;
    ...
    if ( n < 20 )
    {
        ...
        B = hlib_matrix_build_ccoeff( bct, kernel, & h, HLIB_LRAPX_SVD,
                                      acc2, 1, & info );                        CHECK_INFO;
        
        printf( "  done in %.1f seconds\n", hlib_walltime() - start );
        hlib_matrix_print_ps( B, "cbem1d_A_svd", HLIB_MATIO_SVD, & info );   CHECK_INFO;
    ...
    x = hlib_vector_cbuild( n, & info );                                        CHECK_INFO;
    b = hlib_vector_cbuild( n, & info );                                        CHECK_INFO;

    /*
     * set up rhs
     */

    b_arr = (hlib_complex_t *) malloc( n * sizeof(hlib_complex_t) );

    for ( i = 0; i < n; i++ )
        b_arr[i] = hlib_complex( rhs( i, n ), 0 );

    hlib_vector_cimport( b, b_arr, & info );                               CHECK_INFO;
    ...
    if ( 1 )
    {
        ...
        B  = hlib_matrix_copy_acc( A, acc, 0, & info );                     CHECK_INFO;
        LU = hlib_matrix_factorise_inv( B, acc, & info );                   CHECK_INFO;

        printf( "    done in %.1f seconds\n", hlib_walltime() - start );

        hlib_matrix_print_ps( B, "cbem1d_LU", HLIB_MATIO_SVD, & info ); CHECK_INFO;
        ...
        if ( n < 20 )
        {
            ...
            hlib_matrix_mulvec( 1.0, A_D, x, -1.0, b2, HLIB_MATOP_NORM, & info );
    ...
    if ( 1 )
    {
        ...
        IA = hlib_matrix_copy_acc( A, acc, 0, & info );                     CHECK_INFO;
        hlib_matrix_inv( IA, acc, & info );                                 CHECK_INFO;
        
        printf( "    done in %.1f seconds\n", hlib_walltime() - start );
        
        hlib_matrix_print_ps( IA, "cbem1d_I", HLIB_MATIO_SVD, & info );  CHECK_INFO;
